...
console.log("**** roomList[0] is: " + roomList[0].roomId);
	matrixClient.joinRoom(viewingRoom.roomId).done(function(room) {
                    setRoomList();
                    viewingRoom = room;
                    printMessages();
		    matrixClient.sendMessage(viewingRoom.roomId, "Initializing Bot...\n");
                    rl.prompt();
                }, function(err) {
...
matrixClient.on("syncComplete", function() {
    setRoomList();
	viewingRoom = roomList[0];
console.log("**** roomList[0] is: " + roomList[0]);
	matrixClient.joinRoom(viewingRoom.roomId).done(function(room) {
                    setRoomList();
                    viewingRoom = room;
                    printMessages();
...
matrixClient.on("Room.timeline", function(event, room, toStartOfTimeline) {
	...
	if (viewingRoom && event.getSender() != myUserId) {
		lastEvent = event;
		if (event.getContent().body === "getList")
		{
			matrixClient.sendTextMessage(viewingRoom.roomId, event.getSender() +" -> " + buyingList);
		}
		else
		{
    			matrixClient.sendTextMessage(viewingRoom.roomId, event.getSender() +" -> " + parseMessage(event.getContent().body));
		}
...
function parseMessage(message) {
	...
	try
	{
		var lala = JSON.parse("[" + message + "]");
		console.log (lala);
	   	buyingList = message.split(",");
		return "ok";
	}
	catch(e)
	{
...
function submit()
{	
	//console.log("timeline length: " + viewingRoom.timeline.length);
	var currentList = [];
	if (viewingRoom.timeline.length > 0)
	{
		currentList = viewingRoom.timeline[0];
	}
	
	
	var newValue = document.getElementById("newElement").value;
	currentList.splice(currentList.length, 0, newValue);
console.log("FUUU: " + currentList);
...
console.log(newList);
	...
	for (var i = 0; i < arrayLength; i++) {
    		tableList += "<div> " + newList[i] + " </div> ";
    
	}

//tableList += "<div> " + element + " </div> ";
    	list.innerHTML = tableList;
	matrixClient.sendTextMessage(viewingRoom.roomId, deserialize(newList));
}

function deserialize(newList)
{
...
function printLine(event) {
    ...
    if (event.getSender() === myUserId) {
        name = "Me";
        separator = ">>>";
        if (event.status === matrixcs.EventStatus.SENDING) {
            separator = "...";
            //fmt = clc.xterm(8);
        }
        else if (event.status === matrixcs.EventStatus.NOT_SENT) {
            separator = " x ";
            //fmt = clc.redBright;
        }
